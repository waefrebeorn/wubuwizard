...
| **Qwen3.5 role split** (AN61) | GGUF-CORRECTED 2026-08-09: the fused attn_qkv 6144 = the GDN's qk(4096)+v(2048) with attn_gate 2048 = the z (the in_proj_qkvz 8192 split across two tensors); the gated-attn is UNFUSED (attn_q 4096 = q+gate, attn_k/v 512, shared q/k norms 256); layer kinds from the config (6×[3 GDN+1 gated]) | `make test_qwen35` + `make test_qwen35_forward` + `make wubu_qwen35_probe` (REAL-GGUF: blk.3 attn_q 4,194,304 elems = 1024×4096 confirmed on disk; both branches finite + deterministic); the 0.8B GGUF local (811MB); the full-stack logit-parity vs llama.cpp is next |
| **Run-compare** (AN62) | two event logs → loss/suite/accept-rate/violations deltas + a tie-band verdict (any violation increase = hard veto) | `make wubu_runcompare` — run3 beats run2 (suite mean +0.096, accept rate +0.356), self-compare ties |
| **Anomaly detector** (AN63) | the release gate: suite-while-violating, flat-loss-with-accepts, degenerate accept-all, skills-without-suite, resume re-stamps; **caught the demo runner's monotone loss → gate never rejected (rate 1.000)** → seeded ±0.06 jitter fixes it (256 accept/44 reject, replay-verified) | `make wubu_anomaly` — clean on the fixed run; wired into the nightly gate as check #5 |
| **Disk-full drill** (AN64) | `ulimit -f` simulates ENOSPC mid-write; the pre-kill sidecars stay intact (atomic save), the resume restores, the torn trailing event is tolerated | `./tools/disk_full_drill.sh` — PASSED |
| **Clock-skew resume** (AN65) | kill+resume monotonicity: resume-batch check + post-kill rounds ≥ base, ≤1 torn line, no >2x, mtime + loss continuity; **caught a REAL bug: hdr.batch was saved but never restored → the resume re-stamped from 1** | `./tools/clock_skew_resume.sh` — PASSED (the batch-restore fix + the O_DIRECTORY portability + test_diagnosis' max_cells==seed deadlock + teardown double-free all fixed; 10/10 gates green) |
