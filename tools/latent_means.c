...
int main(int argc, char **argv) {
    ...
    for (int s = 0; s < steps; s++) {
        ...
        /* eps channel means BEFORE and AFTER cfg */
        double em[4] = {0,0,0,0}, nm[4] = {0,0,0,0};
        for (int i = 0; i < 4*64*64; i++) { em[i/4096] += eps[i]; nm[i/4096] += nc[i]; }
        for (int c = 0; c < 4; c++) { em[c] /= 4096; nm[c] /= 4096; }
        for (int i = 0; i < 4*64*64; i++) eps[i] = nc[i] + 7.5f * (eps[i] - nc[i]);
        double cm[4] = {0,0,0,0};
        for (int i = 0; i < 4*64*64; i++) cm[i/4096] += eps[i];
        for (int c = 0; c < 4; c++) cm[c] /= 4096;
        ...
        printf("s%d t=%3d: eps[%+.3f %+.3f %+.3f %+.3f] uncond[%+.3f %+.3f %+.3f %+.3f] cfg[%+.3f %+.3f %+.3f %+.3f] -> x[%+.3f %+.3f %+.3f %+.3f]\n",
               s, t, em[0],em[1],em[2],em[3], nm[0],nm[1],nm[2],nm[3], cm[0],cm[1],cm[2],cm[3], xm[0],xm[1],xm[2],xm[3]);
    }
    return 0;
}
