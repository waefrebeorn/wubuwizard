...
def extract_glm51(path, out, n_out):
    """GLM-5.1-Reasoning-1M jsonl: input + output with <think>...</think>.
    The reasoning trace is KEPT WHOLE (OpenThoughts Appx H: stripping
    self-reflection destroys small-model reasoning)."""
    import json as _json
    with open(path) as f:
        for line in f:
            line = line.strip()
            ...
            try:
                ...
                continue
            q = clean(obj.get("input", ""))
            if len(q) < 8: continue
            out.write("[PROBLEM]\n" + q + "\n")
            outp = clean(obj.get("output", ""))
            if not outp: continue
            # split <think>...</think> from the final answer
            ti, tj = outp.find("<think>"), outp.find("</think>")
            if ti >= 0 and tj > ti:
                think = outp[ti+7:tj].strip()
                rest = outp[tj+8:].strip()
                if len(think) > 4:
                    out.write("[REASONING]\n" + think + "\n")
                if len(rest) > 4:
                    out.write("[ANSWER]\n" + rest + "\n")
...
def extract_ot3(path, out, n_out):
    """OpenThoughts3-1.2M: conversations (ChatML-wrapped, QwQ-32B teacher).
    The messages list holds the full reasoning trace + answer."""
    t = pq.read_table(path)
    col = "conversations" if "conversations" in t.column_names else t.column_names[0]
    for i in range(t.num_rows):
        v = t.column(col)[i].as_py()
        if not v: continue
        doc = []
...
def main():
    ...
    jobs = [
        ...
        ("/home/wubu/models/corpus/reasoning/glm-5.1-reasoning-1m/**/*.jsonl",
