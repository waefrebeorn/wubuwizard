...
- AN47 ENDURANCE RUN + CHECKPOINT-RESTART `wired` (2026-08-09: --resume loads the .hive + .prio sidecars (wubu_diag_load restores the ledger + counters, the resumed cells replay into the hive); --ckpt-hive N saves the sidecars mid-run so a kill resumes from the kill step; PROVEN: 8-step run -> resume -> 'resumed 3 fitness cells' + 4 more batches continue; test_colony PASSES — the release gate: harness floor + zero contract violations + prio sidecar + blueprint bounds, 'no ship without it')
- AN48 PER-FAMILY GEMV DISPATCH `wired` (include/wubu_gemv_dispatch.h + src/wubu_gemv_dispatch.c — the precision ladder EXECUTES: >=16 bits -> F32/BF16, <16 -> INT8 quantized; wubu_disp_reladder switches the backend when the diagnose changes the plan (re-quantize, not paper); test_gemv_dispatch PASSES: dense 8->INT8, reladder 8->32 switches, INT8 vs F32 rel err 0.0049)
- AN49 LIVE-FILE HARNESS TASKS `wired` (include/wubu_harness_file.h + src/wubu_harness_file.c — REAL file work: byte-exact text-chunk round-trip + the Kodak codec PCM->image->PCM scored by the mid-window shape correlation (0.707); outcomes are traj cells; test_harness_file PASSES)
- AN50 SKILL -> TRAIN FEEDBACK `wired` (include/wubu_skill_train.h + src/wubu_skill_train.c — accepted skills push PREFERENCE PAIRS, high-value traj cells push SFT segments; the bounded stream DRAINS into the next train window: experience -> weights, not only experience -> hive cells; test_skill_train PASSES)
- AN51 COLONEL END-TO-END EFFECT `wired` (tools/test_colonel_effect.c — the full chain PROVEN: specialist -> Colonel request (9P path in the subtree) -> toolreg gate (deny-default: unregistered denied) -> the EFFECT (a real file lands in the namespace) -> traj cell + cap accounting; specialists act without the host)
...
- AN61 QWEN3.5 HYBRID LOADER ROLE SPLIT `wired` (the AN28 open question RESOLVED from the HF config + NVIDIA bridge + the justinchuby GDN analysis: include/wubu_qwen35.h + src/wubu_qwen35.c — the fused-tensor role math driven by the config, NOT hardcoded. Gated-attn attn_qkv [d,6144] = q_and_gate(4096) + k(512) + v(512) + z(1024) — the extra 1024 is the OUTPUT-GATE tail (attn_output_gate: true), NOT a missing head split; the GDN ssm_in_proj [d,8192] = qk(4096) + v(2048) + z(2048), conv 6144, out 4096. Layer kinds from the config's layer_types: 24 layers = 6 x [3 GDN + 1 gated] (gated at 3,7,11,15,19,23). test_qwen35 pins ALL the 0.8B numbers. The loader's fused-split + per-layer kind are now computable; the forward + logit-parity remain the next step once the 0.8B GGUF is on disk) GGUF-CORRECTED SAME DAY: the 0.8B GGUF downloaded to /home/wubu/models/Qwen3.5-0.8B-Q8_0.gguf (811MB) shows the fused attn_qkv [1024,6144] lives on the GDN layers (18 of 24) as qk(4096)+v(2048) with attn_gate [1024,2048] = the z (the config's in_proj_qkvz 8192 = attn_qkv 6144 + attn_gate 2048, split across TWO GGUF tensors); the gated-attn layers (3,7,11,15,19,23) are UNFUSED: attn_q [1024,4096] = q+gate, attn_k/v [1024,512], shared per-head attn_q_norm/attn_k_norm [256], attn_output [2048,1024]. The config-only inference (fused-6144-as-gated-attn with an output-gate z tail) was WRONG — the GGUF is the source of truth. wubu_qwen35.h/.c + the forward (wubu_qwen35_forward.h/.c, both branches) corrected + test_qwen35/test_qwen35_forward pin the GGUF numbers. REAL-GGUF PROBE WIRED: tools/wubu_qwen35_probe.c loads the ACTUAL Q8_0 tensors (gguf_reader, 763MB mmap) and runs BOTH forward branches against them — blk.3 attn_q 4,194,304 elems = 1024x4096 (the unfused layout CONFIRMED on disk), both outputs finite + deterministic (rerun diff 0.00e+00). The forward-to-GGUF wiring is verified; the full 24-layer stack + embedding + head + logit-parity vs llama.cpp (llama-cli local at /home/wubu/llama.cpp) is the loader's next step.
- AN62 RUN-COMPARE `wired` (A7: tools/wubu_runcompare.c — two event logs -> the delta report: loss/suite curves, accept rate, violations, window ratios + a tie-band verdict (ends within 0.002 -> the mean decides; ANY violation increase is a hard veto). Verified: run3 (honest) beats run2 (scripted) on suite mean +0.096 + accept rate +0.356; self-compare ties)
- AN63 ANOMALY DETECTOR + THE NOISY-LOSS FIX `wired` (A8: tools/wubu_anomaly.c — the release gate scanning one log for: suite-while-violating windows, flat-loss-with-accepts (the scripted/gamed gate), degenerate accept-all/reject-all, skills-without-suite-progress, resume re-stamps. IT CAUGHT A REAL ARTIFACT: the demo runner's loss was perfectly monotone -> the gate ALWAYS accepted (rate 1.000); fixed with SEEDED loss jitter (+/-0.06, deterministic so the replay still agrees) -> the gate now genuinely decides (300 rounds: 256 accept / 44 reject, rate 0.853, replay-verified 0 divergences). Wired into the nightly gate as check #5)
- AN64 DISK-FULL DRILL `wired` (A9: tools/disk_full_drill.sh — `ulimit -f` caps the events file, the write fails exactly like an ENOSPC disk-full (SIGXFSZ mid-write); asserts the pre-kill .hive/.prio/.skills are INTACT (the atomic tmp+fsync+rename — a kill mid-save leaves the OLD archive, never a torn one), the resume restores, the torn trailing event is tolerated, a clean run still works. PASSED)
- AN65 CLOCK-SKEW RESUME + A REAL BUG `wired` (A10: tools/clock_skew_resume.sh — kill + resume + monotonicity: the AUTHORITATIVE check is the resume's own batch (the runner logs 'resume continues at round batch+1'); the events sanity check validates the POST-KILL rounds are >= the resume base, at most ONE torn line (the group-commit tear), no round >2x, mtime non-decreasing, no loss resets. THE DRILL CAUGHT A REAL BUG: hdr.batch was SAVED in the archive header but never RESTORED on load — a resume re-stamped the round counter from 1 (the stream went 86 -> 2). Fixed: wubu_diag_load restores loop->batch = hdr.batch. ALSO the O_DIRECTORY GNU-extension portability fix (strict -std=c11 hides it) + a REAL test regression found by the full sweep: test_diagnosis' max_cells==seed (8==8) deadlock made the gate stasis forever since the DA wave (the amoeba_live_cells fix made it visible) — fixed to max_cells=16 + the teardown double-free (the hive clear freed the amoeba's registry cells twice once the colony could grow) fixed with a skip-aware hive clear. All gates green)
